...
def create_execution_settings_tab(
    ...
    def browse_file():
        chosen = filedialog.askopenfilename(
            title="Select a File",
            filetypes=[
                ("Supported Files", ("*.tif", "*.tiff", "*.qt", "mov.movie.qt", "*.m4v", "*.mov", "*.MOV", "*.mp4", "*.nd2")),
                ("Movie Files", ("*.mov", "*.MOV", "*.mp4", "*.qt", "*.m4v")),
        ...
        if (chosen[-4:] == ".mov") or (chosen[-4:] == ".mp4") or (chosen[-4:] == ".MOV") or (chosen[-4:] == ".qt"):
            def movie_to_tif(video_path, output_tif_path):
                ...
                tifffile.imwrite(output_tif_path, frames)

                #print(f"Success! Saved {len(kept_frames)} frames to {output_tif_path}")

            # Example Usage
            movie_Name = chosen
            output_path = f'{movie_Name[:-4]}.tif'
            movie_to_tif(movie_Name, output_path)

            config_data["gui_execution"]["file_path"].set(output_path)
